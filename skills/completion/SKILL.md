...
PR作成の**前に**以下のゲートチェックを実行。すべてPassするまでPR作成不可:

| ゲート | 確認方法 | Pass条件 |
|-------|---------|---------|
| ドキュメント更新 | `git diff --name-only` にドキュメントファイルが含まれる | 関連ドキュメントが更新されている |
| CLAUDE.md最適化 | 行数確認: `wc -l CLAUDE.md` | 200行以内 |
| memory記録 | ファイル存在確認 | Phase完了時の学びが記録されている |
