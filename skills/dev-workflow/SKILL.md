...
### Normal Mode（変更なし）
通常実行モード（model: "sonnet"等）では既存のフローをそのまま使用する。
以降の全セクションはNormal Modeを前提とした記述であり、Plan Mode固有の動作は
各セクションの「Plan Mode固有」注記を参照すること。
...
### 自律判断原則（タスクの進め方はClaudeが決定）

**タスクの実行アプローチについてユーザーに確認しない。** Claudeが自律的に判断して進める。

**Claudeが自律的に決定すること:**
- どのサブエージェントを使うか
- Phase遷移のタイミング（設計承認を除く）
- 調査・実装の進め方・順序
- エラー発生時のリカバリー方法
- Chrome調査を実施するかどうか
- タスク分割の粒度
...
**ユーザーに質問すること:**
- 要件（何を作るか・MVP境界・受け入れ条件・非機能要件）← 徹底的に質問する
- Phase 3の設計成果物承認（API仕様・DBスキーマ）← 唯一の実行ゲート

**Red Flags（やってはいけない）:**

| 思考 | 現実 |
|------|------|
| "実装アプローチをユーザーに確認しよう" | 自律判断せよ。ユーザーは任せている |
| "このPhaseを進めていいか聞こう" | 自動遷移せよ（Phase 3承認を除く） |
| "どのエージェントを使うか聞こう" | スキルの定義通りに自律選択せよ |
| "Chrome調査をするか聞こう" | モードとタスク内容から自律判断せよ |
...
### F. HTMLモック（new-creationのみ）
...

### G. 整合性チェックリスト（existing-modificationのみ）
...
...
- docs/design/{wfId}-test-design.md ← Phase 3Cのテスト設計
